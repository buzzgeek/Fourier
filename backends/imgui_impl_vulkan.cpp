...
struct ImGui_ImplVulkan_Data
{
    ...
    VkDeviceMemory              UploadImageBufferMemory;
    VkBuffer                    UploadImageBuffer;
...
bool ImGui_ImplVulkan_CreateImageTexture(VkCommandBuffer command_buffer)
{
    ...
    pixels = stbi_load(".\\me.png", &width, &height, &channels, STBI_rgb_alpha);
    ...
    // Create the Image:
    {
        ...
        err = vkCreateImage(v->Device, &info, v->Allocator, &bd->Image);
        check_vk_result(err);
        VkMemoryRequirements req;
        vkGetImageMemoryRequirements(v->Device, bd->Image, &req);
        VkMemoryAllocateInfo alloc_info = {};
        alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        alloc_info.allocationSize = req.size;
        alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, req.memoryTypeBits);
        err = vkAllocateMemory(v->Device, &alloc_info, v->Allocator, &bd->ImageMemory);
        check_vk_result(err);
        err = vkBindImageMemory(v->Device, bd->Image, bd->ImageMemory, 0);
        check_vk_result(err);
    }
    ...
    // Create the Image View:
    {
        ...
        err = vkCreateImageView(v->Device, &info, v->Allocator, &bd->ImageView);
        check_vk_result(err);
    }

    // Create the Descriptor Set:
    bd->ImageDescriptorSet = (VkDescriptorSet)ImGui_ImplVulkan_AddTexture(bd->ImageSampler, bd->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    // Create the Upload Buffer:
    {
        ...
        buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        err = vkCreateBuffer(v->Device, &buffer_info, v->Allocator, &bd->UploadImageBuffer);
        check_vk_result(err);
        VkMemoryRequirements req;
        vkGetBufferMemoryRequirements(v->Device, bd->UploadImageBuffer, &req);
        bd->BufferMemoryAlignment = (bd->BufferMemoryAlignment > req.alignment) ? bd->BufferMemoryAlignment : req.alignment;
        VkMemoryAllocateInfo alloc_info = {};
        alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        alloc_info.allocationSize = req.size;
        alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
        err = vkAllocateMemory(v->Device, &alloc_info, v->Allocator, &bd->UploadImageBufferMemory);
        check_vk_result(err);
        err = vkBindBufferMemory(v->Device, bd->UploadImageBuffer, bd->UploadImageBufferMemory, 0);
        check_vk_result(err);
    }
    ...
    {
        char* map = NULL;
        err = vkMapMemory(v->Device, bd->UploadImageBufferMemory, 0, upload_size, 0, (void**)(&map));
        check_vk_result(err);
        memcpy(map, pixels, upload_size);
        VkMappedMemoryRange range[1] = {};
        range[0].sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
        range[0].memory = bd->UploadImageBufferMemory;
        range[0].size = upload_size;
        err = vkFlushMappedMemoryRanges(v->Device, 1, range);
        check_vk_result(err);
        vkUnmapMemory(v->Device, bd->UploadImageBufferMemory);
    }

    // free the stbi object
    ...
    // Copy to Image:
    {
        ...
        vkCmdCopyBufferToImage(command_buffer, bd->UploadImageBuffer, bd->Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    ...
    myData.height = static_cast<float>(height);
    myData.width = static_cast<float>(width);
    myData.textureId = (ImTextureID)bd->ImageDescriptorSet;
    //// Store our identifier
    //io.UserData = (void *)bd->ImageDescriptorSet;
    io.UserData = (void*) & myData;
...
static void ImGui_ImplVulkan_CreateImageSampler(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd->ImageSampler)
        return;
    ...
    VkResult err = vkCreateSampler(device, &info, allocator, &bd->ImageSampler);
    check_vk_result(err);
}
...
void    ImGui_ImplVulkan_DestroyFontUploadObjects()
{
    ...
    //TODO
    if (bd->UploadImageBuffer)
    {
        vkDestroyBuffer(v->Device, bd->UploadImageBuffer, v->Allocator);
        bd->UploadImageBuffer = VK_NULL_HANDLE;
    }
    if (bd->UploadImageBufferMemory)
    {
        vkFreeMemory(v->Device, bd->UploadImageBufferMemory, v->Allocator);
        bd->UploadImageBufferMemory = VK_NULL_HANDLE;
    }
}
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    // TODO
    if (bd->ImageView) { vkDestroyImageView(v->Device, bd->ImageView, v->Allocator); bd->ImageView = VK_NULL_HANDLE; }
    if (bd->Image) { vkDestroyImage(v->Device, bd->Image, v->Allocator); bd->Image = VK_NULL_HANDLE; }
    if (bd->ImageMemory) { vkFreeMemory(v->Device, bd->ImageMemory, v->Allocator); bd->ImageMemory = VK_NULL_HANDLE; }
    if (bd->ImageSampler) { vkDestroySampler(v->Device, bd->ImageSampler, v->Allocator); bd->ImageSampler = VK_NULL_HANDLE; }
